...
So glad you asked, wise student. 
...
The second model ($y = A$) is said to be *nested* within the other model ($y = A + B$). Let's look at a non-nested model:
...
That's good, right? Well, no. The problem is that we can add *anything* to the model to improve fit. We can improve our prediction of temperature by adding my daily weight to our prediction model. It will improve the fit (very slightly). But we know that my weight has nothing to do with the temperature. So, if our model is filled with predictors that really have nothing to do with our outcome of interest, we're adding useless information.
...
Now we return to those four magical words: Is it worth It? When we ask our statistical models that questions, we're asking whether the improvement in fit is worth the cost of potentially overfitting. 
...
*Traditionally, testing for an interaction asks whether the interaction effect differs from zero. A model comparison asks if a model which allows the slopes to differ is substantially better than a model that does not allow the slopes to differ. 
...
Are you at all surprised that I begin with visualizations? You probably shouldn't be.
...
So, in short, this function will visually compare two models. In the plot above, one in blue is the reduced model and the red one is the full model. 

So, what are we looking for when viewing two plots in compare.fits?

We are looking for *different predictions*. In this case, the two models have similar predictions, at least in some instances. For example, both models predict ptsd scores near 8 when injuries are low (0-3) for the south battlefield. However, they have a larger difference in their predictions for the north battle field (maybe 8 versus 7.7). 
...
Let's look at another example with a more drastic difference. This time, we'll fit a model with/without injuries (while leaving north_south in both):

```{r modcompav2, echo=TRUE}
require(flexplot)
full    = lm(ptsd~north_south + injuries, data=avengers)
reduced = lm(ptsd~north_south           , data=avengers)
compare.fits(ptsd~injuries | north_south, 
             data=avengers, 
             model1 = full,
             model2 = reduced)
```

Now our two models are *drastically* different. For example, the left-most plot says the full model predicts a ptsd score over 8 for those with no injuries, while the reduced model  predicts a score just over 7.
...
The AIC is short for Akaike's Information Criteria. It is defined as:
...
BIC tends to be more conservative than the AIC. (In other words, the BIC will generally favor the reduced model moreso than the AIC). Fortunately, the two usually agree. 

Like the AIC, the numbers associated with the BIC cannot be interpreted in isolation. Rather, we use the BIC from two models and favor the model with the lower BIC. 
...
By convention, Bayes factors larger than around 10 are considered fairly strong evidence in favor of that model. One thing you'll have to remember about the BF is that anything near 1 is *ambiguous*. For some reason (likely the extensive training we all receive in p-values), it's tempting to interpret a BF of, say 1.5, as saying the reduced model should be favored. This is not the case! Rather, 1.5 says the BF is having a really hard time deciding between the two. 
...
Back when I was talking about `compare.fits`, I was trying, by eye, to tell you how different two model's predictions are. For example, I said Figure \@ref(fit:modcompav2) predicted over 8 versus over 7 for the north battlefield/0 injuries folks for the full/reduced model, respectively. 

Perhaps, it might be a smidge better to have more concrete values than having to eyeball things. That's what these predicted value are. 

Bascially, what `model.comparison` does in the background is it uses both models to predict scores for each individual. For example, the full model might predict a ptsd score of 7.6, while the reduced model might predict a ptsd score of 5.4. Then `model.comparison` will compute the difference in prediction. In this example (7.6 versus 5.4), the difference will be 2.2. It will compute this difference for *all* scores in the dataset. The predicted_difference section tells you the percentiles of these differences. 
...
This is telling you that the *minimum* difference between the two models is `r prediff[1]`. It also says that 25% of scores differ by no more than `r prediff[2]`, 50% differ by no more than `r prediff[3]`, 75% differ by no more than `r prediff[4]`, and that maximum difference in prediction is `r prediff[5]`. Remember that the ptsd scale ranges from about 5 to 10, so a difference of around 0.05 is really tiny. In other words, these two models are generating *very* similar predictions. 
...
You may not like the answer, but I'm going to tell you anyway. In those cases, it's best to say that the answer is ambiguous. The reality is that the evidence isn't strong either way. So, my best recommendation is to report that the evidence is ambiguous, then collect more data. 
...
Statistics is really terrible at determining whether a mdoel fits. It's really good at telling you which of two models fit better. There are two types of models: nested models (meaning we can delete one part of a model to get the other model), and non-nested models. 
